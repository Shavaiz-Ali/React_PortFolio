...
export const Navbar = () =>{
    ...
    return(
        <>
        <div className="container w-full mx-auto md:py-5 py-6 ">
            ...
            <div className="logo md:py-0 py-2">
                ...
                <div className="toggle-btn">
                    <span onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
                        {isMobileMenuOpen ? <FaTimes className="text-white md:hidden text-[2.2rem] absolute top-[0.5rem]  right-[5px] cursor-pointer"/>:<RxHamburgerMenu className="text-white md:hidden text-[2.2rem] absolute top-[0.5rem] right-[5px] cursor-pointer"/> }
                    </span>
                </div>
                <ul className={`flex md:flex-row flex-col items-center text-center md:gap-10 gap-5 absolute bg-black md:static md:z-auto z-40 md:top-[20px] md:w-auto w-full left-0 py-8 md:border-hidden border border-white/[0.15] transition-all  ease-in-out ${isMobileMenuOpen ? "top-[4rem]" : "top-[-490px]"}`}>
                    { 
                        links.map((item, ind) =>{
                            return(
                                < >
